...
class DetectionThread:
    ...
    def __init__(self, predictor, text_encodings):
        ...
        self.boxes = []  # list of [x1, y1, x2, y2] in pixel coords
        self._running = True
        self._thread = threading.Thread(target=self._run, daemon=True)
        self._thread.start()
...
def main():
    ...
    try:
        while True:
            ...
            # Encode box coordinates as JSON bytes
            boxes_json = json.dumps(boxes).encode() if boxes else b"[]"

            # Send: [4B frame_size][4B boxes_size][1B bike_flag][JPEG][boxes_json]
            header = struct.pack(">IIB", len(data), len(boxes_json), int(detector.bike_detected))

            if conn is not None:
                try:
                    conn.sendall(header + data + boxes_json)
            ...
            frame_idx += 1
            time.sleep(0.033)  # cap at ~30fps to avoid flooding the link
